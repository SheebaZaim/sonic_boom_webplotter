...
# #     parser.add_argument('--out', nargs='?', const='',  # <-- if --out is used without value, const='' is assigned, default=None,
...
# # args = parser.parse_args()  # First parse arguments

# # # Now access args.out safely
# # if args.out == '' or args.out is None:
# #     # Auto-assign from mapping
# #     base_input = os.path.splitext(os.path.basename(args.input))[0].lower()
# #     mapping = {
...
# def load_csv(csvfile):
#     """Load CSV and return t, p arrays."""
#     df = pd.read_csv(csvfile)
#     return df['t'].values, df['p'].values

# def compute_metrics(reference_p, calc_p):
#     """Compute RMSE and peak error metrics."""
#     err = calc_p - reference_p
#     rmse = np.sqrt(np.mean(err ** 2))
#     ref_peak = np.max(np.abs(reference_p))
#     calc_peak = np.max(np.abs(calc_p))
...
#     # --- Safe Resampling to prevent MemoryError ---
#     fs_req = params.get('fs_req', 200000.0)
#     max_points = 5000  # Maximum allowed points
#     t_uniform = np.linspace(t[0], t[-1], min(max_points, len(t)))
#     p_uniform = np.interp(t_uniform, t, p)
...
#     parser.add_argument('--reference', required=False, help="Reference CSV file")
#     parser.add_argument('--distance', type=float, default=15760.0, help="Propagation distance")
#     parser.add_argument('--out', nargs='?', const='', help="Optional output prefix")
#     args = parser.parse_args()

#     # --- Determine output prefix ---
#     if args.out == '' or args.out is None:
#         base_input = os.path.splitext(os.path.basename(args.input))[0].lower()
#         mapping = {"fig4.4": "fig4.5", "fig8b": "fig10"}
#         out_prefix = mapping.get(base_input, base_input)
#     else:
#         out_prefix = args.out
...
# Comments
# --------
# ✔ Auto-matching of output to target figure
# ✔ Comparison plot saved
# ✔ Summary generated automatically
# """
